#!/bin/bash
# Run GPU Layout for all the human chromosomes, and report its run time in the log file. 
...
LOG_FILE="${WORK_DIR}/run_gpu_layout_all.log"
if [ -f "${LOG_FILE}" ]; then
    rm ${LOG_FILE}
fi
...
echo "===== Run GPU Layout for all the human chromosomes =====" 2>&1 | tee -a ${LOG_FILE}
for chroms in ${CHR_NAMES[@]}; do
    ...
    LAY_FILE="${DATASET_DIR}/layouts_gpu/${chroms}.lay"
    PNG_FILE="${DATASET_DIR}/images_gpu/${chroms}.png"

    set -x
    # [odgi layout]
    ${ODGI_BIN} layout -i ${OG_FILE} -o ${LAY_FILE} -X ${PATH_FILE} --threads ${NUM_THREAD} \
    --gpu 2>&1 | tee -a ${LOG_FILE}
    # [odgi draw]
